...
/**
 * A scheduled function to be run monthly to calculate the previous month's recap for all users.
 */
export const onMonthlyRecap = async () => {
  ...
  for (const basicUserInfo of allUsers) {
    ...
    if (!user.userGoal || !user.userProfile?.timezone) {
      console.log(`Skipping monthly recap for user ${user.uid}: not enough data.`);
      continue;
    }
    ...
    if (mealEntries.length === 0 || !targets) {
      console.log(`Skipping monthly recap for user ${user.uid}: no meals or targets.`);
      continue;
    }
